...
// Optional params
$templateName = isset($input['template']['name']) ? $input['template']['name'] : null;
$messageText  = isset($input['text']) ? $input['text'] : null;
$parameters = $input['template']['components'][0]['parameters']; // your {{lead}} text
...
try {
    if ($message_type === "template") {
        ...
        $res = sendWhatsAppMessage($authToken, $receiver_number, $templateName,$parameters, $version, $phone_number_id);
  ...
  writeLogg("-------------------------------------------------------------------------------------------" );
        ...
        echo json_encode(["status" => "success", "message" => "Text message sent successfully", "response" => $res, "Res"=>$resDecoded]);
